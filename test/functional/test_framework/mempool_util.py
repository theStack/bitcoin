...
def fill_mempool(test_framework, node, miniwallet):
   ...
   -maxmempool=5.
    ...
    separate tagged ephemeral miniwallet instance.
    """
    ...
    assert ephemeral_miniwallet.get_scriptPubKey() != miniwallet.get_scriptPubKey()
    test_framework.generate(miniwallet, 1)
    test_framework.generate(ephemeral_miniwallet, num_of_batches * tx_batch_size)
    ...
    with node.assert_debug_log(["rolling minimum fee bumped"]):
        for batch_of_txid in range(num_of_batches):
            ...
            create_lots_of_big_transactions(ephemeral_miniwallet, node, fee, tx_batch_size, txouts)
