...
static void VerifyScriptBench(benchmark::Bench& bench, ScriptType script_type)
{
    ...
    // Create key material needed for output script creation / signing
    ...
    switch (script_type) {
    case ScriptType::P2WPKH: scriptPubKey = GetScriptForDestination(WitnessV0KeyHash(pubkey)); break;
    case ScriptType::P2TR:   scriptPubKey = GetScriptForDestination(WitnessV1Taproot(XOnlyPubKey{pubkey})); break;
    ...
    PrecomputedTransactionData txdata;
    {
        ...
        txdata.Init(txSpend, std::move(spent_outputs));
    }
