...
void PeerManagerImpl::MaybeProcess1P1CPackage(const CTransactionRef& ptx, NodeId nodeid)
{
    ...
    if (tx_orphan) {
        LogDebug(BCLog::TXPACKAGES, "found child %s (wtxid=%s) of tx %s (wtxid=%s) in orphanage, trying package evaluation\n",
                 tx_orphan->GetHash().ToString(), tx_orphan->GetWitnessHash().ToString(), parent_txid.ToString(), parent_wtxid.ToString());
        const Package package_1p1c{ptx, tx_orphan};
        ProcessPackageResult(package_1p1c,
            ProcessNewPackage(m_chainman.ActiveChainstate(), m_mempool, package_1p1c, /*test_accept=*/false, /*max_sane_feerate=*/std::nullopt),
            nodeid);
    } else {
